...
async function atualizarEstados() {
  try {
    const semaforoResp = await fetch(`${API_BASE}/semaforo`)
    const semaforoData = await semaforoResp.json()
    atualizarSemaforo(semaforoData.dados.semaforo)

    try {
      const modoResp = await fetch(`${API_BASE}/iluminacao/modo`)
      if (modoResp.ok) {
        const modoJson = await modoResp.json()
        if (modoJson.modo) {
...
async function alterarModoSemaforo(modo) {
  try {
    const resp = await fetch(`${API_BASE}/semaforo/modo`, {
      method: "POST",
...
async function alterarModoIluminacao(modo) {
  modoIluminacao = modo
  try {
    const resp = await fetch(`${API_BASE}/iluminacao/modo`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ modo }),
    })
